...
class UserBehavior(TaskSet):
    ...
    def login(self):
        ...
        ret = self.client.post("/api/user/auth", {'req': msgpack.packb(requestParams), 'seq': self.seq})
        self.seq += 1
    ...
    def my_task(self):
        print('task start')
        ret = self.client.post("/api/quest/list", {'seq': self.seq})
        self.seq += 1
        ret2 = msgpack.unpackb(ret.content, ext_hook=ext_hook, encoding='utf-8')
        ret3 = msgpack.unpackb(ret2[2], ext_hook=ext_hook, encoding='utf-8')
        print('解凍後のdataを更に解凍:', ret3)

        #レスポンスを1,2,3ではなくプロパティにする方法
        #共通部分の抜き出し
